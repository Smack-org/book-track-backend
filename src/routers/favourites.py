...
async def get_favourites(offset: int = 0, limit: int = 20,
    ...
    result = await db.execute(
        text("""SELECT book_id, created_at FROM favourite_books WHERE
...
async def add_favourite(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Book {book.book_id} not found in Gutendex")
...
async def remove_favourite(
    ...
    await session.execute(
        text("DELETE FROM favourite_books WHERE user_id = :user_id AND book_id = :book_id"),
